...
/// removable drive; disabled with clear textual reason ("Job not yet
/// complete" / "Waiting for verification") until the underlying job has
/// completed AND every file is verified.
///
...
class EraseDriveActionButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<List<JobFile>>(
      ...
      builder: (context, snapshot) {
        ...
        final reason = _eligibilityReason(files);
        ...
        return Tooltip(
          message: reason ?? 'Erase the source SD card',
  ...
  String? _eligibilityReason(List<JobFile> files) {
    if (job.status != JobStatus.completed) {
      return 'Job not yet complete';
    }
    final unverified =
        files.isEmpty || files.any((f) => !f.verified);
    if (unverified) {
...
/// + warning content. Surfaces snackbars on the caller's `ScaffoldMessenger`.
Future<bool> eraseSourceDrive(BuildContext context, Job job) async {
