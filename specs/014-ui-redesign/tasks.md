...
- [ ] T056 [US6] Extract `_eraseSourceDrive()` flow from `lib/ui/screens/job_detail_screen.dart` into new `lib/ui/widgets/erase_drive_action.dart` — preserves `_showEraseConfirmDialog`, serial-number identity check, typed drive-path field, size-only warning
- [ ] T057 [US6] Wire the EraseDriveAction button into the `JobCardActive` header slot (created in T020) for transfer-type jobs whose source is a removable drive; disabled with reason text ("Waiting for SHA-256 verification" / "Job not yet complete") until eligible. **Erase remains in the Active card header**, not inside DetailTabs — visibility is independent of expansion state (FR-018: always visible).
- [ ] T058 [US6] **Verification checkpoint**: confirm via code review that the extracted EraseDriveAction widget preserves all safety gates with no regression. Acceptance: serial-number identity check fires before AND after typed confirmation; size-only warning is shown when applicable; dialog aborts with "Drive changed" snackbar if the drive changes between open and confirm.
- [ ] T059 [US6] Create `lib/ui/widgets/job_card_completed.dart`: ephemeral celebration card shown in the active slot when queue transitions from running to all-done. Persists for the green-dot lifetime (5min or until next operator action — driven by `QueueStateNotifier` from T012). Two CTAs: [Erase Cards] [New Job].
- [ ] T060 [US6] Mount `JobCardCompleted` in `lib/ui/screens/home_screen.dart` reacting to `QueueStateNotifier.onQueueAllDone`. Same notifier event also fires the StatusBar 5-minute timer (T015), so the celebration card and the green dot clear together.
- [ ] T061 [US6] Implement [Erase Cards] CTA in JobCardCompleted as a sequential per-card flow: for each detected card from the just-completed batch, the existing typed-confirmation dialog (via EraseDriveAction) fires once. CTA NEVER bulk-erases (Constitution Principle I, FR-012). Operator can cancel mid-sequence; already-erased cards stay erased.
