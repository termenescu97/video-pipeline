...
- **Q1 — F-1 schema impact**: Where does `Job.sourceDriveSerial` live? → **A: New schema migration v8 → v9** with explicit `addColumn`. **Codex round-27a P1 correction**: the original "null = legacy bypass" rule was a backdoor — null also surfaces under transient WMI flake at v9 create-time, accidentally granting bypass to v9 jobs that should fail-closed. **Revised rule**: the v8→v9 migration backfills existing rows with a sentinel string `'__legacy_v8__'`; v9 job-creation paths (`createBatchTransferJobs`, single-job creation) refuse to create the job if `getDriveIdentity` returns null, surfacing "could not identify SD card — re-insert and retry." After 019 ships, `null` is impossible (would indicate a bug); `'__legacy_v8__'` means "legacy bypass, surface one-time-per-launch banner"; any other value means "real serial, check normally."
...
- **FR-001**: A `Job.sourceDriveSerial` (TEXT, nullable) column MUST be added to the `jobs` table. The v8→v9 migration MUST backfill all existing rows with the sentinel `'__legacy_v8__'`. v9 job-creation paths (`createBatchTransferJobs` and single-job creation in `create_job_screen.dart`) MUST refuse to create the job if `DriveService.getDriveIdentity()` returns null/empty; the operator sees "could not identify SD card — re-insert and retry." Post-019, `null` in this column is a bug indicator; `'__legacy_v8__'` is the legacy-bypass marker; any other value is a real serial.
- **FR-002**: At every transfer-resume, the executor MUST re-call `getDriveIdentity()` for the current `Job.sourcePath` drive letter. Branching: (a) `Job.sourceDriveSerial == '__legacy_v8__'` → surface one-time-per-launch legacy banner per job-id, then proceed without serial check (preserves in-flight v8 work); (b) `Job.sourceDriveSerial == null` → log error (this is a post-019 bug indicator) and refuse to proceed; (c) `Job.sourceDriveSerial == real serial` AND current returns null → fail-closed (refuse, "could not verify card identity"); (d) `Job.sourceDriveSerial == real serial` AND current matches → proceed; (e) `Job.sourceDriveSerial == real serial` AND current differs → refuse with "card identity mismatch" banner.
- **FR-003**: At erase-eligibility evaluation in `eraseEligibilityReason()`, the same five-branch logic from FR-002 MUST run; mismatch / null-current / null-stored cases produce a refusal that prevents the typed-confirmation dialog from opening.
- **FR-004**: At v9 job-create-time, the system MUST fail-closed on `getDriveIdentity()` null result by refusing job creation entirely. This is the load-bearing invariant that closes the Codex round-27a P1 backdoor — without it, null in `sourceDriveSerial` would ambiguously mean both "legacy" and "v9 capture failed."
