...
- **FR-019**: Slack compression-completed notifications for chained compression (i.e. compression jobs created via auto-chain from a `transferAndCompress` parent job) MUST also surface the parent transfer's verify counts (verified / unverified / mismatch). The architectural mechanism is a new `Job.parentJobId` column (v8 schema, nullable, populated by `_createChainedCompressionJob` at chain time, never modified afterward). At notification time, the chained compression job queries its parent via `parentJobId` and includes the parent's verify-count snapshot in the Slack body with a warning prefix when non-clean. For directly-created `compression` jobs (no parent), `parentJobId` is null and the Slack body shows compression-only metrics. This closes the cross-phase notification gap — a compression-complete Slack ping after a non-clean transfer must NOT look clean.
