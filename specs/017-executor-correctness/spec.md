...
- **JobFile**: Represents a single source-destination file pair within a Job. Carries copy state (pending / copying / copied / failed), verification state (pending / verified / mismatch / unverified), and failure kind (none / copyError / verifyMismatch / verifyUnreliable) for retry routing.
