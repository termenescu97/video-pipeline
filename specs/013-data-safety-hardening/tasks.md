...
- [ ] T029 [US2] Split `createBatchTransferJobs()` in `lib/services/job_queue_service.dart` into two methods: (1) `preflightBatchTransfer(List<DetectedDrive> drives, String destination, VerificationMode mode)` that builds the complete file list for ALL cards (with per-card subfolders), checks all destination paths for existing files, and returns `({List<({DetectedDrive drive, String subfolder, List<JobFilesCompanion> files})> cards, List<String> conflicts})`. (2) `executeBatchTransfer(List<({...})> resolvedCards, ...)` that takes the resolved (post-conflict) card list and creates jobs atomically via `createJobWithFiles()`. Cards with zero files after resolution are skipped
- [ ] T030 [US2] Wire batch conflict preflight to UI in `lib/ui/screens/home_screen.dart` — call `preflightBatchTransfer()` first. If `conflicts` is non-empty, show `ConflictResolutionDialog`. Apply resolution to the card/file list (skip: remove conflicting files; rename: update destination paths with suffix; newFolder: re-pick and re-preflight; overwrite: proceed as-is; cancel: abort). Then call `executeBatchTransfer()` with the resolved list. If all files across all cards were removed, show "All files already exist" message and skip batch creation entirely
